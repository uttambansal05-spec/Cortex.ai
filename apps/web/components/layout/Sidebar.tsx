    ...
    <aside className="w-52 flex-shrink-0 flex flex-col h-full bg-surface border-r border-border">
        ...
        {NAV.map(({ href, label, icon: Icon, exact }) => {
          ...
          return (
            <Link key={href} href={href} className={cn(
              ...
              active ? 'bg-surface-3 text-foreground' : 'text-foreground-2 hover:text-foreground hover:bg-surface-2'
            )}>
      ...
      <div className="px-2 py-3 border-t border-border space-y-0.5">
        <Link href="/dashboard/settings" className={cn(
          ...
          pathname === '/dashboard/settings' ? 'bg-surface-3 text-foreground' : 'text-foreground-2 hover:text-foreground hover:bg-surface-2'
        )}>
          <Settings className="w-3.5 h-3.5" />
          Settings
        </Link>
        <button onClick={handleSignOut} className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded text-sm text-foreground-2 hover:text-foreground hover:bg-surface-2 transition-colors">
          <LogOut className="w-3.5 h-3.5" />
          Sign out
        </button>
        <div className="mt-2 px-2.5 py-2 flex items-center gap-2">
          <div className="w-5 h-5 rounded-full bg-accent/20 flex items-center justify-center flex-shrink-0">
            <span className="text-2xs text-accent font-medium">{user?.email?.[0]?.toUpperCase()}</span>
          </div>
