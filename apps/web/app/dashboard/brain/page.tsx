...
export default function BrainMapPage() {
  ...
  const renderGraph = async () => {
    ...
    const filteredEdges = edges.filter(e => filteredNodeIds.has(e.from_node) && filteredNodeIds.has(e.to_node))

    const simNodes = filteredNodes.map(n => ({ ...n }))
    const linkMap = new Map(simNodes.map(n => [n.id, n]))
    const simLinks = filteredEdges.map(e => ({
      ...e,
      source: linkMap.get(e.from_node) || e.from_node,
      target: linkMap.get(e.to_node) || e.to_node,
    }))
    ...
    const sim = d3.forceSimulation(simNodes as any)
      .force('link', d3.forceLink(simLinks).id((d: any) => d.id).distance(90).strength(0.35))
    ...
    const link = g.append('g').selectAll('line')
      ...
      .attr('stroke', (d: any) => COLORS[d.source?.node_type] || '#888')
    ...
    node.append('circle')
      .attr('r', (d: any) => Math.max(5, 5 + (degree[d.id] || 0) * 0.8))
      .attr('fill', (d: any) => COLORS[d.node_type] || '#888')
      .attr('fill-opacity', 0.8)
      .attr('stroke', (d: any) => COLORS[d.node_type] || '#888')
      .attr('stroke-width', 1.5)
      .attr('stroke-opacity', 0.35)

    node.append('text')
      .text((d: any) => d.label.length > 16 ? d.label.slice(0, 15) + '…' : d.label)
      .attr('dy', (d: any) => -(Math.max(5, 5 + (degree[d.id] || 0) * 0.8)) - 5)
      ...
      <div style={{ flex: 1, display: 'flex', position: 'relative', overflow: 'hidden' }}>
        {loading ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12 }}>
            <Loader2 style={{ width: 20, height: 20, color: '#7F77DD', animation: 'spin 1s linear infinite' }} />
        ...
        {/* Help text */}
        {nodes.length > 0 && (
          <div style={{ position: 'absolute', bottom: 20, right: selectedNode ? 296 : 20, fontSize: 10, color: '#444441' }}>
        ...
        {/* Node detail panel */}
        {selectedNode && (
          <div style={{ width: 260, background: '#17171e', borderLeft: '0.5px solid #2e2e3a', padding: 20, overflowY: 'auto', flexShrink: 0 }}>
